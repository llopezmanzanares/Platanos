...
# TODO: considerar la estacionalidad de la siguiente manera, 100% la media de cada anaulidad, y hacer los porcentajes de variación respecto a ese valor, idea sacada de estas gráficas https://thecrudechronicles.substack.com/p/seasonal-ceasfire
...
# consumos por bimestre
bimestres_m3 <- 
  balten_ds$bimensuales |> 
  filter(year(fecha) > 2020) |> 
  select(fecha, consumo_m3) |> 
  mutate(
    aa = as_factor(year(fecha)),
    mm = month(fecha, label = TRUE),
    # mm_txt = as_factor(mm),
    m_act = month(today(), label = TRUE),
    transparencia = if_else(m_act > mm, .9, 0)
  ) 

# media del consumo hasta el bimestre en curso
media_bimestre <- 
  bimestres_m3 |> 
  filter(mm <= m_act) |> 
  select(aa, mm, consumo_m3) |> 
  summarise(.by = aa, cons = sum(consumo_m3))
...
bimestres_m3 |>  
  ...
  geom_bar(
    aes(
      weight = consumo_m3, 
      fill = lvls_reorder(mm, 12:1), 
      alpha = transparencia
      )
    ) +
  geom_hline(data = media_bimestre, aes(yintercept = median(cons))) +
  labs(
    title = "Acumulado anual del consumo de agua y media del <br>acumulado hasta el bimestre actual (m<sup>3</sup>)",
  ...
  scale_y_continuous(position = "right", labels = label_number(big.mark = ".", decimal.mark = ","))
